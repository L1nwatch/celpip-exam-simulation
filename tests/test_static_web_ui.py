...
class StaticWebUiTests(unittest.TestCase):
    ...
    def test_speaking_choice_step_is_selectable_without_recording(self):
        ...
        self.assertIn("const speakingChoiceReview = speakingChoiceStep && Boolean(state.submissions.speaking);", app_js)
        self.assertIn('speakingChoiceReview ? "" : `<div class="speaking-recorder">', app_js)
        self.assertIn('"Choice step — no recording"', app_js)
        self.assertIn('"This step only saved your option for the next speaking task."', app_js)
        self.assertIn('class="speaking-choice-selector"', app_js)
        self.assertIn("async function bindSpeakingChoiceSelector(card, question, status, reviewOnly = false)", app_js)
        self.assertIn("if (choiceReview) bindSpeakingChoiceSelector(card, q, null, true);", app_js)
        self.assertIn('${reviewOnly ? "disabled" : ""}', app_js)
        self.assertIn("if (!reviewOnly) buttons.forEach", app_js)
    ...
    def test_each_listening_review_transcript_is_collapsed_by_default(self):
        ...
        self.assertIn('aria-controls="sourceContent" aria-expanded="true"', index_html)
